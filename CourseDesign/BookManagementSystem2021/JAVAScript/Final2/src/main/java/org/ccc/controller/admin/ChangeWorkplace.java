...
@WebServlet("/ChangeWorkplace")
public class ChangeWorkplace extends HttpServlet
{
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, IOException, ServletException
    {
        ...
        int workplace_id=Integer.parseInt(request.getParameter("updatewid"));
        String workplace_name = request.getParameter("workplace_name2");
        ...
        System.out.println(workplace_name);
        System.out.println(linkman.isEmpty());
        System.out.println(telephone.isEmpty());
        System.out.println(e_mail.isEmpty());
        System.out.println(address.isEmpty());
        System.out.println(character.isEmpty());

        if(workplace_name.isEmpty() || linkman.isEmpty() || telephone.isEmpty() || e_mail.isEmpty() || address.isEmpty() || character.isEmpty())
        {
        ...
        int hasWorkplace=0;	//1表示已经有该单位了，0表示没有可以
        for(Workplace workplace:workplaces) {
            //单位名相同，但不是同一个记录
            if(workplace.getWorkplace_name().equals(workplace_name)&&workplace.getWorkplace_id()!=workplace_id) {
                hasWorkplace=1;
                break;
            }
        }
        if(hasWorkplace==1) {
            response.getWriter().write("<script language=javascript>alert('提示，单位已存在，不可修改成该单位');window.location='/admin/WorkplaceManagement.jsp'</script>");
            return;
        }

        //获取当前的workplace
        Workplace workplace=workplaceDao.selectWorkplaceByWorkplaceId(workplace_id);
